...
const DealersOrders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const dealerId = localStorage.getItem('dealerId'); // Retrieve user ID from cookies
        
        if (!dealerId) {
          throw new Error('User not logged in');
        }
        
        const response = await axiosInstance.get(`/api/v1/orderdata/orders/dealers/${dealerId}`);
        ...
        <ul>
          ...
          orders.map(order => ( 
            <li key={order._id} className="border-b mb-4 pb-4">
              ...
              <p className="text-lg font-semibold text-gray-800">Order from( users ID): {order.userId}</p>
            </li>
            </li>
          ))}
        </ul>
